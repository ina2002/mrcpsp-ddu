...
class Benders:
    ...
    def __init__(self, instance, Gamma, time_limit, cost=None, e_over=0):
        """
        ...
        # Master problem model and variables. Populated in create_masterproblem and updated when solved.
    ...
    def solve(self, num_threads=4, print_log=False, master_type='compact'):
        ...
        if master_type == 'compact':
            self.create_compact_masterproblem(num_threads, print_log)
        elif master_type == 'balouka':
            self.create_balouka_masterproblem(num_threads, print_log)
        while self.LB < self.UB:
            ...
            if time_remaining <= 0:
                ...
                if self.master_model.status == 9:  # if time_limit reached whilst in master problem, terminate
                    ...
                    robust_makespan = float(self.sub_model.ObjVal)   # 一定是数值

                    V = self.instance.V
                    M = [self.instance.jobs[j].M for j in V]

                    # ✅ 用 .X 取值，确保 fixed_cost 是 Python float
                    fixed_cost = 0.0
                    for i in V:
                        for idx, m in enumerate(M[i]):
                            if self.master_x[i, m].X > 0.5:          # 选中的模式
                                fixed_cost += float(self.cost[i][idx])

                    total_obj = fixed_cost + float(self.e_over) * robust_makespan
                    self.sub_objval = float(total_obj)               # ✅ 关键：强制是 float
                    if print_log is True:
    ...
    def print_masterproblem_sol(self):
        """
    ...
    def create_compact_masterproblem(self, num_threads=4, print_log=False):
        """
        ...
        # 计算 cost[i,m] 逻辑: sum_k (r[i][m][k] * d[i][m])
        #cost = [[sum(r[i][m][k] for k in range(len(r[i][m]))) * d[i][m] for m in M[i]] for i in V]
        ...
        model.setObjective(gp.quicksum(cost[i][m] * x[i, m] for i in V for m in M[i]) + e_over * eta, GRB.MINIMIZE)
    ...
    def create_balouka_masterproblem(self, num_threads=4, print_log=False):
        """
        Creates Gurobi model to represent basic master problem without optimality cuts. The master problem selects job
        processing modes and resource allocations to minimise project duration under nominal job processing times. The
        ...
        # set model objective
        # 目标函数：模式选择成本 + 工期成本 (e_over * eta)
        cost_map = {(i, m): cost[i][idx] for i in V for idx, m in enumerate(M[i])}
        model.setObjective(
gp.quicksum(cost_map[(i, m)] * x[i, m] for i in V for m in M[i]) + e_over * eta,
    GRB.MINIMIZE
)
        # Precedence constraints
